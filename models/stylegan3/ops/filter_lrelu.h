...
template <class T, class index_t, bool signWrite, bool signRead> filtered_lrelu_kernel_spec choose_filtered_lrelu_kernel(const filtered_lrelu_kernel_params& p, int sharedKB)
{
    ...
    // printf("fushapex fushapey %d %d fdshapex fdshapey %d %d p.up p.down %d %d\n", p.fuShape.x, p.fuShape.y, p.fdShape.x, p.fdShape.y, p.up, p.down);
...
//         @staticmethod
//         def forward(x, fu, fd, b, si, sx, sy): # pylint: disable=arguments-differ
